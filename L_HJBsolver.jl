...
function value_iterationHJB(V0, Ft, p)
    ...
    for it in 1:p.maxitHJBvalue
        ...
        # HJB operator
        Vnew, wnew = T_HJB((VS = VS, VI = VI, VC = VC, VR = VR), Ft, p; w0 = wnew)
        ...
        if p.verbose && (it % 50 == 0 || it == 1)
            ...
            println("iter = $it, error = $err, step = $step, ω = $(p.ω), w = $wnew")
        end
        ...
        if err < p.tolHJBvalue
            ...
            return (VS = VS, VI = VI, VC = VC, VR = VR, w = wnew)
        end
    end
...
function T_HJB(V, Ft, p; w0)
    ...
    # real wage from fixed point iteration
    w = fixed_point_wage(V,(∂kVS=∂VS_log, ∂kVI=∂VI_log, ∂kVC=∂VC_log, ∂kVR=∂VR_log),Ft,p; w0 = w0)
    ...
    # HJB for Infected (I)
    for i in 1:p.Nk
        transition_I_to_C = p.σ1 * (VC[i] - VI[i])
        transition_I_to_R = p.σ3 * (VR[i] - VI[i])
        ...
        VI_new[i] = (flow_utility + capital_term + transition_I_to_C + transition_I_to_R + transition_I_to_S) / p.ρ
    end
    ...
    for i in 1:p.Nk
        ...
        transition_C_to_S = (p.αEpi + p.μ) * (VS[i] - VC[i])
    ...
    return (VS = VS_new, VI = VI_new, VC = VC_new, VR = VR_new), w
end
